...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        int cur_index = index;
        while (parent != 0) {
            if (contents[parent].priority() > contents[cur_index].priority()) {
                swap(parent, cur_index);
            } else {
                break;
            }
            cur_index = parent;
    ...
    private void sink(int index) {
        ...
        while (curIndex <= size / 2) {
            minIndex = curIndex;
            if (left <= size) {
                if (contents[curIndex].priority() > contents[left].priority()) {
                    minIndex = left;
                }
            }
            if (right <= size) {
                if (contents[right].priority() < contents[minIndex].priority()) {
                    minIndex = right;
                }
            }
